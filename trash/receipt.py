...
class ReceiptConfig(Config):
    """Configurations for training on the datasets.
    Derivers form the base Config class and overrides some values.
    """
    # Give the configurations a recognizable name 
    NAME = "receipt"
...
class ReceiptDataset(utils.Dataset):
    def load_receipt(self,data_dir, subset):
    ...
    def load_mask(self,image_id):
        """Generate instance masls for an image
        Returns:
            masks: A bool array of shape [height,width,instance count] with
            one mask per instance.
            class_ids: a 1D array of class IDs of the instance masks
        """
        #if not a receipt dataset image, delegate to parent class.
...
def train(model):
    ...
    dataset_val.load_receipt(args.dataset, "val")
    dataset_val.prepare()
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Train Mask R-CNN to detect Receipts.')
    parser.add_argument("command",
                        metavar="<command>",
                        help="'train' or 'splash'")
    parser.add_argument('--dataset', required=False,
                        metavar="/path/to/Receipt/dataset/",
